  ...
  Value = (function() {
    ...
    Value.prototype.cast = function(to_type, explicit) {
      ...
      switch (this.type) {
        case 'null':
          if (explicit) {
            if (!(to_type in this.LOLCoffee.DEFAULT_VALUES)) {
              throw new MachineError('Unknown type: ' + to_type);
            }
            result = this.LOLCoffee.DEFAULT_VALUES[to_type];
          } else {
            if (to_type !== 'bool') {
  ...
  Parser = (function() {
    ...
    Parser.prototype.parseDeclaration = function() {
      ...
      if (this._nextIs('keyword', 'ITZ')) {
        ...
        if (this._nextIs('keyword', 'A')) {
          this._consume();
          value = this.LOLCoffee.DEFAULT_VALUES[this.parseType()];
    ...
    Parser.prototype.parseExpression = function() {
      ...
      if (this._nextIs('keyword')) {
        ...
        } else if (__indexOf.call(this.LOLCoffee.UNARY_OPERATORS, keyword) >= 0) {
          return this.parseUnaryExpression();
        } else if (__indexOf.call(this.LOLCoffee.BINARY_OPERATORS, keyword) >= 0) {
          return this.parseBinaryExpression();
        } else if (__indexOf.call(this.LOLCoffee.INFINITARY_OPERATORS, keyword) >= 0) {
    ...
    Parser.prototype.parseType = function() {
      ...
      if (!(type in this.LOLCoffee.DEFAULT_VALUES)) {
    ...
    Parser.prototype.parseUnaryExpression = function() {
      ...
      if (__indexOf.call(this.LOLCoffee.UNARY_OPERATORS, operator) < 0) {
    ...
    Parser.prototype.parseBinaryExpression = function() {
      ...
      operator = this._consume('keyword');
      if (__indexOf.call(this.LOLCoffee.BINARY_OPERATORS, operator) < 0) {
    ...
    Parser.prototype.parseInfinitaryExpression = function() {
      var args, operator;
      operator = this._consume('keyword');
      if (__indexOf.call(this.LOLCoffee.INFINITARY_OPERATORS, operator) < 0) {
...
app.get('/', function (req, res) {
  res.send('Make a POST request to /interpretLOLCODE with the parameters "input" and "code" to run LOLCODE!');
})

app.post('/interpretLOLCODE', function (req, res) {
  console.log(req.body);
  function replaceAll(string, search, replace) {
  return string.split(search).join(replace);
}
        var code=req.body.code;
      code=replaceAll(code, '(DOUBLEQUOTE)', '"');
            code=replaceAll(code, '(NEWLINE)', '\n');
            var tokenized = new LOLCoffee.Tokenizer(code).tokenize();
            var parsed = new LOLCoffee.Parser(tokenized).parseProgram();
         codegen_context = new LOLCoffee.CodeGenContext();
            parsed.codegen(codegen_context);   var machine = new LOLCoffee.Machine(codegen_context, function() {
            machine.resume(req.body.input);
          }, function(text) {
res.send(text);
            machine.resume();
          }, function(error) {
